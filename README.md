...
同时，我们为您添加了一个`main.cpp`、两个建议类`Commit`和
`Repository`，以及`main.cpp`所依赖的命令门面类`SomeObj`（骨架中其声明与实现均为空，需要您自行补全），以帮助入门；除此之外，你需要自己编写`CMakeList.txt`完成这个项目的编译与运行, 为了评测的正常运行，我们要求您编译后生成的可执行文件需要放在
`gitlite/build` 目录下.
...
6. 每一个`commit`命令都应当由其 SHA-1
   id 标识，其内容必须包含其文件的`blob`引用、父引用（可以不止一个，参见命令`merge`）、日志消息和提交时间。
...
ID。提交中显示的时间戳反映的是**当前时区**，而不是 UTC；因此，初始提交的时间戳不是 1970 年 1 月 1 日星期四 00:00:00，而是等效的北京时间。输出时请保证其按照`EEE MMM dd HH:mm:ss yyyy Z`的格式输出（其中`dd`表示日期按两位补零，如`01`），项与项之间用空格隔开，注意前导0的实现；
...
`SHA-1` 标识符的情况下，你应该同样支持对缩写形式的支持，即提交 id 的任意长度前缀都可以作为其缩写（缩写形式不一定为六位数字，上面只是一种可能的例子）。
...
如果分割点（`split point`,
`also lowest common ancestor (LCA)`）与给定分支是同一个提交，那么我们什么都不做；我们宣布合并完成，打印消息`Given branch is an ancestor of the current branch.`
后退出；如果分割点是当前分支，则执行快进（fast-forward）：当前分支的指针前移到给定分支的头提交（**HEAD 仍指向当前分支，不切换分支**），用该提交跟踪的文件覆盖工作目录、删除其中不存在于该提交的跟踪文件，并清空暂存区，然后打印消息`Current branch fast-forwarded.`后退出；快进同样需要进行下述的未跟踪文件检查。否则，我们继续以下步骤：
...
在本子任务中，你可以选择完善`status`功能，或者完成`remote`功能（包括`add-remote`,`rm-remote`,`push`,`fetch`,`pull`）命令, 或者两者兼有。评测样例中`6-status-05`（5 分）与`6-remote-01`~`6-remote-03`（各 5 分）均计入总分，因此若想取得 bonus 满分，两者都需要完成。
...
在该命令中，你需要将指定的远程仓库地址保存在指定的远程名称下，例如`gitlite add-remote other ../testing/otherdir/.gitlite`。您的程序需要将所有正斜杠`/`转换为路径分隔符用以正确读取路径（在 Linux 上正斜杠即为路径分隔符，无需额外处理）。

除此之外，我们要求：如果给定名称的远程已经存在，则打印错误消息：`A remote with that name already exists.`后退出。您不必检查用户名和服务器信息是否合法，也无需检查该路径是否存在——即使路径不存在，`add-remote`也应当静默成功；路径问题延迟到`fetch`/`push`/`pull`时才报告`Remote directory not found.`。
...
将远程仓库中指定分支的所有提交和
`blob`（当前仓库中尚未存在的）复制到本地仓库中`.gitlite`中的分支，该分支的名字为`[remote name]/[remote branch name]`，并更改`[remote name]/[remote branch name]`使其指向头提交（从而将远程仓库中分支的内容复制到当前仓库）。如果此分支之前不存在，则会在本地仓库中创建。
...
对于 Code
Style 部分，我们会根据你的`README.md`以及代码布局与风格进行给分，包括但不限于适当的注释，合理的空行，优秀的板块设计等等，占 5 `pts`.优秀者可适度给6-7 `pts`.
