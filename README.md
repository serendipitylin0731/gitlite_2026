...
`Repository`，以及`main.cpp`所依赖的命令门面类`SomeObj`（骨架中已给出与`main.cpp`匹配的方法签名，实现为空，需要你自行补全），以帮助入门；除此之外，你需要自己编写`CMakeLists.txt`完成这个项目的编译与运行, 为了评测的正常运行，我们要求你编译后生成的可执行文件需要放在 `gitlite/build` 目录下.
...
二、某些命令会触发失败情况，并会指定错误消息。这些错误消息的具体格式将在规范的后续部分中指定。所有错误消息都以句点`.`结尾。如果程序遇到这些失败情况之一，它必须打印错误消息，并且不得更改任何其他内容。当多个失败情况同时满足时，按规范中列出的顺序报告第一个。除了列出的失败情况之外，你无需处理任何其他错误情况。
...
在该命令中，你需要将暂存区中已跟踪的文件保存为一个新的提交（`commit`）。该提交被称为**跟踪**已保存的文件，它记录了这些文件的快照，并为它们创建一个新的历史版本，便于日后恢复。每个提交通常会附带一条提交信息（`message`），描述这次提交的内容。如果提交信息包含空格等多个单词，请用引号将其括起来作为一个参数传入，例如`gitlite commit "Two files"`。
...
1. 与`log`只显示`commit tree`的某个子树不同，`global-log`要求显示所有`commit`的信息，甚至包括`reset`到某个旧提交后不再属于任何分支的提交。`commit`的顺序无关紧要。
...
具体地，该命令要求满足格式上各部分之间有一个空行，整个状态后无空行结尾。条目应按字符串字典序列出（星号不计算）；其中`Modifications Not Staged For Commit`一节按文件名字典序列出，即排序时不计入`(modified)`/`(deleted)`后缀。
...
在该命令中，你需要用`reset`重置当前分支的`HEAD`指针。具体地，该命令要求满足检出（`checkout`）指定提交所跟踪的所有文件,移除该提交中不存在的跟踪文件,同时将当前分支的头移动到该提交节点。执行该命令后，暂存区将被清空。你的程序不需支持对commit-id的缩写功能（`checkout`支持缩写是因为它面向日常交互；`reset`通常与完整 id 配合使用，故不作要求，支持了也不影响评测）。
...
如果分割点（`split point`，即最近公共祖先 LCA）与给定分支是同一个提交，那么我们什么都不做；我们宣布合并完成，打印消息`Given branch is an ancestor of the current branch.`
...
8. 任何在当前分支和指定分支中以不同方式修改的文件都属于冲突文件。“以不同方式修改”可能意味着两个文件的内容均已更改且彼此不同，或者其中一个文件的内容已更改而另一个文件被删除，或者该文件在分割点不存在，并且在指定分支和当前分支中的内容不同。在这种情况下，请将工作目录中冲突文件的内容替换为（请在具体冲突的地方进行替换）
...
值得注意的是，每一个合并提交与其他提交不同：它们会将当前分支的头（称为“第一个父级”）和命令行中指定的待合并分支的头都记录为父级，在寻找分割点的过程中一定要注意与常规LCA算法的区别。
...
`Gitlite`中的远程仓库与本地仓库别无二致；由于调试能力限制，我们采用保存在本地的非当前仓库作为远程仓库。提取其分支时，本地分支名形如`origin/master`（即包含`/`），请确保你的分支存储与解析能正确处理这种带`/`的分支名。

---

#### `add-remote`（用法：`gitlite add-remote [remotename] [name of remote directory]/.gitlite`）

在该命令中，你需要将指定的远程仓库地址保存在指定的远程名称下，例如`gitlite add-remote other ../testing/otherdir/.gitlite`。你的程序需要将所有正斜杠`/`转换为路径分隔符用以正确读取路径（在 Linux 上正斜杠即为路径分隔符，无需额外处理）。
...
2. 在这种情况下，请将这些新提交追加到远程分支，然后将远程分支的`HEAD`重置为与本地分支的头提交相同；
...
将远程仓库中指定分支的所有提交和
`blob`（当前仓库中尚未存在的）复制到本地仓库，并在本地仓库中创建或更新名为`[remote name]/[remote branch name]`的分支，使其指向远程分支的头提交。
...
1. 只比较被提交跟踪的文件：工作目录中未跟踪的文件不参与比较，暂存区不影响比较结果。
...
3. 格式细节：`---`行为旧版本，`+++`行为新版本；新增文件的`---`行为`/dev/null`，删除文件的`+++`行为`/dev/null`；hunk 头为`@@ -旧起始行,旧行数 +新起始行,新行数 @@`（某一侧为空时起始行为 0）；每条变更保留前后各 3 行上下文（前缀为空格），删除行前缀为`-`，新增行前缀为`+`；若某行不以换行符结尾，则在该行之后额外输出一行`\ No newline at end of file`。
...
你需要提交一个`README.md`来说明你的项目的设计思路，其可以包括：
