...
ID。提交中显示的时间戳反映的是**当前时区**，而不是 UTC；因此，初始提交的时间戳不是 1970 年 1 月 1 日星期四 00:00:00，而是等效的北京时间。输出时请保证其按照`EEE MMM dd HH:mm:ss yyyy Z`的格式输出，项与项之间用空格隔开，注意前导0的实现；
...
`SHA-1` 标识符的情况下，你应该同样支持对缩写形式的支持（缩写形式不一定为六位数字，上面只是一种可能的例子）。
...
在该命令中，你需要用`reset`重置当前分支的`HEAD`指针。具体地，该命令要求满足检出（`checkout`）指定提交所跟踪的所有文件,移除该提交中不存在的跟踪文件,同时将当前分支的头移动到该提交节点。执行该命令后，暂存区将被清空。你的程序不需支持对commit-id的缩写功能。
...
（将`contents of file in ... branch`替换为指定文件的内容）并自动暂存结果（将分支中已删除的文件视为空文件）。请注意行终止符和行分隔符的运用，不注意这一点的人将会度过一个失败的人生：）
...
值得注意的是，每一个合并提交与其他提交不同：它们会将当前分支的头（称为“第一个父级”）和命令行中指定的待合并分支的头都都记录为父级，在寻找分割点的过程中一定要注意与常规LCA算法的区别。
